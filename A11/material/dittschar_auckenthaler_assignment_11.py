        ...
        for m in np.arange(1, n):
            for j in np.arange(m, n):
                ...
                if len(bilist) > 0:
                    ...
                    if ititer > np.max([itrow, itcol, itdiag]):
                        ptr[i,j]= np.arange(i+1, j)[np.argmax(bilist)]
                else:
                    ititer = 0
...
def traceback(nmat, sequence, structure, i, L):
    ...
    if i < j:
        ...
        if nmat[i,j] == nmat[i+1, j]:
            ...
            print(f"i is {i}\nj is {j}\nFirst letter is: {sequence[i]}\nSEcond letter is {sequence[j]}.")
            structure.append((i,j))#structure,np.array([i, j])[np.newaxis,...], axis=0)
